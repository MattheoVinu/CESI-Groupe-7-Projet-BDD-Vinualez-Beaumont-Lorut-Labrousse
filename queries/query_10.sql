...
select idVille
from ville
where nomVille = 'Toulouse')as t
natural join agence
natural join (
select idCapteur, idAgence
from capteur)as capt
natural join (
select idCapteur, idPersonnel
from releve)as rel
natural join agentadministratif
natural join (select idPersonnel, nomPersonnel, prenomPersonnel, datePoste
from personnel)as persagence
group by idPersonnel) as PersVille
natural join (
